    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        goaldist = 999999999999999
        goalpos = 0

        for temp in livingGhostPositionDistributions:
            pos = temp.argMax()
        ...
        disttogoal = 99999999999999
        nextaction = 0

        for action in legal:
            successorPosition = Actions.getSuccessor(pacmanPosition, action)
            dist = self.distancer.getDistance(successorPosition, goalpos)
